...
def predict_sliding(net, image, tile_size, classes, flip_evaluation, recurrence):
    """
    slide infering with overlapping
    ...
    overlap = 1 / 3  # 1/3 overlapping
    stride = ceil(tile_size[0] * (1 - overlap))  # 512 * 2/3
    # stride + overlap = img_size

    tile_rows = int(ceil((image_size[2] - tile_size[0]) / stride) + 1)  # strided convolution formula
    tile_cols = int(ceil((image_size[3] - tile_size[1]) / stride) + 1)  # +1, last tile
    print("Need %i x %i prediction tiles @ stride %i px" % (tile_cols, tile_rows, stride))

    # full
    full_probs = np.zeros((image_size[2], image_size[3], classes))  # total probs + each tile prediction
    count_predictions = np.zeros((image_size[2], image_size[3], classes))  # add infer cnt for each pixel
    tile_counter = 0
    ...
    for row in range(tile_rows):
        for col in range(tile_cols):
            ...
            x1 = int(col * stride)
            y1 = int(row * stride)
            # bottom right; last overlap can > 1/3; 先定右边，再推出左边，保证每个 tile 都是对的
            ...
            padded_img = pad_image(img, tile_size)  # 当 crop < tile 时
            tile_counter += 1  # count tile

            padded_prediction = net(Variable(torch.from_numpy(padded_img), volatile=True).cuda())
            if isinstance(padded_prediction, list):
                padded_prediction = padded_prediction[0]

            padded_prediction = interp(padded_prediction).cpu().data[0].numpy().transpose(1, 2, 0)  # interp, upsample to tile size
            prediction = padded_prediction[0:img.shape[2], 0:img.shape[3], :]  # crop valid prediction result

            count_predictions[y1:y2, x1:x2] += 1  # count infer times
            full_probs[y1:y2, x1:x2] += prediction  # accumulate the predictions also in the overlapping regions

    full_probs /= count_predictions  # get avg prob for each pixel
    return full_probs
...
def evaluate_main(model, loader, input_size, num_classes, whole=False, recurrence=1, type='val'):
    ...
    for index, batch in enumerate(loader):
        ...
        if type == 'test':
            seg_pred = id2trainId(seg_pred, id_to_trainid, reverse=True)  # train_id 2 ori id

        # if test: only save infer results
        output_im = PILImage.fromarray(seg_pred)
        output_im.putpalette(palette)
        output_im.save('outputs/' + name[0] + '.png')

        # if val: also need cal evaluation metrics
        if type == 'val':
    ...
    if type == 'val':
        ...
        IU_array = (tp / np.maximum(1.0, pos + res - tp))  # each class
