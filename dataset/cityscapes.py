...
class CitySegmentationSD(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_s = Image.fromarray(np.uint8(image_s))
        image_s = data_transforms(image_s)
        image_t = image_t.transpose((2, 0, 1))
        return image_t.copy(), image_s, label.copy(), np.array(size), name


class CitySegmentationDense(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321),
                 mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, use_aug=False, extra_aug=False):
        self.root = root
        self.list_path = list_path
...
class CitySegmentationLightesp(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321),
                 mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, use_aug=False, extra_aug=False):
        self.root = root
        self.list_path = list_path
    ...
    def __getitem__(self, index):
        ...
        pad_w = max(self.crop_w - img_w, 0)
        if pad_h > 0 or pad_w > 0:
            img_pad_s = cv2.copyMakeBorder(image_s, 0, pad_h, 0,
                                           pad_w, cv2.BORDER_CONSTANT,
                                           value=(0.0, 0.0, 0.0))
            img_pad_t = cv2.copyMakeBorder(image_t, 0, pad_h, 0,
                                           pad_w, cv2.BORDER_CONSTANT,
                                           value=(0.0, 0.0, 0.0))
            label_pad = cv2.copyMakeBorder(label, 0, pad_h, 0,
                                           pad_w, cv2.BORDER_CONSTANT,
            ...
            img_pad_s, img_pad_t, label_pad = image_s, image_t, label

        img_h, img_w = label_pad.shape
        h_off = random.randint(0, img_h - self.crop_h)
        w_off = random.randint(0, img_w - self.crop_w)
        # roi = cv2.Rect(w_off, h_off, self.crop_w, self.crop_h);
        image_t = np.asarray(img_pad_t[h_off: h_off + self.crop_h, w_off: w_off + self.crop_w], np.float32)
        image_s = np.asarray(img_pad_s[h_off: h_off + self.crop_h, w_off: w_off + self.crop_w], np.float32)
        label = np.asarray(label_pad[h_off: h_off + self.crop_h, w_off: w_off + self.crop_w], np.float32)
        # image = image[:, :, ::-1]  # change to BGR
        image_s = image_s.transpose((2, 0, 1))
        image_t = image_t.transpose((2, 0, 1))
        ...
        return image_s.copy(), image_t.copy(), label.copy(), np.array(size), name


class CitySegmentationesp(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321),
                 mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, use_aug=False, extra_aug=False):
        self.root = root
        self.list_path = list_path
    ...
    def __getitem__(self, index):
        ...
        # print(name)
        img_h, img_w = label.shape
        image = cv2.resize(image, (int(img_w / 2), int(img_h / 2)), interpolation=cv2.INTER_LINEAR)
        label = cv2.resize(label, (int(img_w / 2), int(img_h / 2)), interpolation=cv2.INTER_NEAREST)
        if self.scale:
            image, label = self.generate_scale_label(image, label)
        image = np.asarray(image, np.float32)
        mean = (73.18188461, 82.93553322, 72.41519599)
        ...
        # #     img_pad,label_pad = image,label
        #
        # h_off = random.randint(0, int(img_h*0.05))
        # w_off = random.randint(0, int(img_w*0.05))
        #
        # img_crop = image[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        # label_crop = label[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        #
        # # image = cv2.resize(img_crop, (self.crop_w,self.crop_h))
        # # label = cv2.resize(label_crop, (self.crop_w,self.crop_h), interpolation=cv2.INTER_NEAREST)
        img_h, img_w = label.shape
        pad_h = max(self.crop_h - img_h, 0)
        pad_w = max(self.crop_w - img_w, 0)
        if pad_h > 0 or pad_w > 0:
...
# class CitySegmentationesptest(data.Dataset):
#     def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321),
#                  mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, use_aug=False, extra_aug=False):
#         self.root = root
#         self.list_path = list_path
...
class CitySegmentationunlabelesp(data.Dataset):
...
class CitySegmentationLightespeval(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321),
                 mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, use_aug=False, extra_aug=False):
        self.root = root
        self.list_path = list_path
...
class CamVidLight(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321),
                 mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, use_aug=False, extra_aug=False):
        self.root = root
        self.list_path = list_path
...
class CitySegmentationLight(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321),
                 mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255, use_aug=False, extra_aug=False):
        self.root = root
        self.list_path = list_path
...
class CitySegmentationTrain(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # label_h = cv2.resize(label, None, fx=1 / 8, fy=1 / 8, interpolation=cv2.INTER_NEAREST)

        w, h = label.shape
        new_shape = (int(np.ceil(w / 8)), int(np.ceil(h / 8)))

        label_h = cv2.resize(label, new_shape, 0, 0, interpolation=cv2.INTER_NEAREST)
        wa, ha = label_h.shape
        label_r = np.reshape(label_h, [1, wa * ha])
        label_c = np.reshape(label_h, [wa * ha, 1])
        attn_H = label_c == label_r
        attn_H = np.asarray(attn_H, np.float32)
        # print(attn_H.shape)

        return image.copy(), label.copy(), attn_H.copy(), np.array(size), name
...
if __name__ == '__main__':
    dst = CitySegmentationLight(root="/teamscratch/msravcshare/v-yifan/",
                                list_path="/teamscratch/msravcshare/v-yifan/SegNet/CamVid/train.txt",
                                crop_size=(480, 360), use_aug=True)
    trainloader = data.DataLoader(dst, batch_size=1, num_workers=0)

    with open("/teamscratch/msravcshare/v-yifan/SegNet/CamVid/val.txt") as f:
        train_list = f.readlines()
    train_list = [x.strip() for x in train_list]

    f_w = open("/teamscratch/msravcshare/v-yifan/SegNet/CamVid/test.txt", "w")
    cnt = np.zeros(19)
